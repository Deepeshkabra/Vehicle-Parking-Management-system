...
class ParkingSpotBase(BaseSchema):
    ...
    vehicle_type: VehicleType = Field(
        VehicleType.FOUR_WHEELER, description="Vehicle type"
    )
...
class ParkingSpotUpdate(BaseSchema):
    ...
    class Config:
        schema_extra = {"example": {"status": "O", "spot_type": "regular"}}
...
class ParkingSpotResponse(ParkingSpotBase, TimestampMixin):
    ...
    current_reservation: Optional[dict] = Field(
        None, description="Current reservation details"
    )
...
class ParkingSpotFilter(BaseSchema):
    ...
    available_only: Optional[bool] = Field(
        None, description="Show only available spots"
    )
