...
class Reservation(db.Model):
    ...
    __table_args__ = (
        db.CheckConstraint(
            "status IN ('active', 'completed', 'cancelled')", name="valid_status"
        ),
        db.CheckConstraint("parking_cost >= 0", name="non_negative_cost"),
        db.CheckConstraint("hourly_rate > 0", name="positive_hourly_rate"),
        db.CheckConstraint(
            "leaving_timestamp IS NULL OR leaving_timestamp > parking_timestamp",
            name="valid_leaving_time",
        ),
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "parking_timestamp": (
                self.parking_timestamp.isoformat() if self.parking_timestamp else None
            ),
            "leaving_timestamp": (
                self.leaving_timestamp.isoformat() if self.leaving_timestamp else None
            ),
            "expected_leaving_time": (
                self.expected_leaving_time.isoformat()
                if self.expected_leaving_time
                else None
            ),
            ...
            "spot_identifier": (
                self.parking_spot.get_spot_identifier() if self.parking_spot else None
            ),
            "lot_name": (
                self.parking_spot.parking_lot.prime_location_name
    ...
    def to_csv_dict(self):
        ...
        return {
            ...
            "vehicle_number": self.vehicle_number,
            "parking_timestamp": (
                self.parking_timestamp.strftime("%Y-%m-%d %H:%M:%S")
                if self.parking_timestamp
                else None
            ),
            "leaving_timestamp": (
                self.leaving_timestamp.strftime("%Y-%m-%d %H:%M:%S")
                if self.leaving_timestamp
                else None
            ),
