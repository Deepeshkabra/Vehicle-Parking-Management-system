...
from utils.error_handlers import (
    ...
    create_success_response, create_error_response, validate_request_data
)
...
def login():
    ...
    try:
        ...
        if (
            data.get('username') == admin_email.lower() and 
        ...
        if not user.is_active:
            raise ForbiddenException("Account is inactive. Please contact administrator.")
...
def validate_and_create_user(user_data):
    ...
    if existing_user:
        raise ValidationException("Username already exists", {"username": ["This username is already taken"]})
    
    # Check if email already exists
    existing_email = User.query.filter_by(email=user_data.email.lower()).first()
    if existing_email:
        raise ValidationException("Email already exists", {"email": ["This email is already registered"]})
